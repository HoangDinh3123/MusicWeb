        ...
        <div class="fixed top-0 w-full h-[56px] shadow-md bg-white lg:hidden">
            <div class="flex justify-between px-4">
                <NuxtLink to="/" class="h-[56px] flex items-center cursor-pointer">
                    ...
                    <div class="text-[20px] text-[#212121] lg:text-[#e5e6e7] font-[1000] ml-1">music</div>
                </NuxtLink>
        ...
        >
            <div class="flex justify-between px-4">
                <div class=" h-[56px] flex items-center">
                    ...
                    <NuxtLink class="text-[20px] text-[#e5e6e7] font-[1000] ml-1">phat</NuxtLink>
                </div>
            </div>

            <div class="h-full w-full">
                <ul class="mx-2">
                    <li class="w-full h-[37.6px] px-2 text-[#8d9195] text-xs flex items-center">Trang chủ</li>
                    <li
                        :class="{'active': selected == 1}"
                        @click="selected = 1"
                        class="hover:bg-[#5f4646] rounded-sm h-[28px] my-[2px] text-[#8c8f93] [&.active]:bg-[#02b875] [&.active]:text-white">
                        <SidebarItem :icon="'ic:baseline-play-circle'" :name="'Khám phá'" :path="'/'"></SidebarItem>
                    </li>
                    ...
                    <li
                        ...
                        @click="selected = 2"
                        class="hover:bg-[#5f4646] rounded-sm h-[28px] my-[2px] text-[#8c8f93] [&.active]:bg-[#02b875] [&.active]:text-white">
                        <SidebarItem :icon="'ic:round-format-align-left'" :name="'Thể loại'" :path="'/browse'"></SidebarItem>
                    </li>
                    ...
                    <li
                        ...
                        @click="selected = 4"
                        class="hover:bg-[#5f4646] rounded-sm h-[28px] my-[2px] text-[#8c8f93] [&.active]:bg-[#02b875] [&.active]:text-white">
                        <SidebarItem :icon="'ic:baseline-account-box'" :name="'Nghệ sĩ'" :path="'/artist/artist'"></SidebarItem>
                    </li>

                    <li
                        @click="openDialog"
                        class="hover:bg-[#5f4646] rounded-sm h-[28px] my-[2px] text-[#8c8f93] cursor-pointer [&.active]:bg-[#02b875] [&.active]:text-white">
                        <SidebarItem :icon="'ic:baseline-search'" :name="'Tìm kiếm'"></SidebarItem>
                    </li>
                </ul>
                ...
                >
                    <div class="px-4 py-3 flex items-center">
                        <img 
                            class="rounded-full h-[32px]" 
                            :src="user.image ? user.image : 'https://flatfull.com/themes/pulse/images/a0.jpg'" alt="">
                    ...
                    <div v-if="isUserMenu" class="absolute top-full w-[80%] bg-white rounded-sm text-[14px]">
                        <div v-if="authStore.getLogged">
                            <NuxtLink to="/user/profile" class="block mt-2 h-[27px] py-[3px] px-[20px]">Trang cá nhân</NuxtLink>

                            <div class="border"></div>

                            <div 
                                @click="logout"
                                class="mt-2 py-[3px] px-[20px]"
                            > 
                                Đăng xuất
                            </div>
                        </div>
